...
# Run tests
test:
	@grind run-tests # @flutter test --concurrency=6 --dart-define=environment=testing --coverage test/
	...
	@docker run --rm -it -v ${PWD}:/app --workdir /app \
		...
		--name flutter_testings \
			plugfox/flutter:stable-web /bin/bash -ci ' \
				...
				&& flutter pub get --suppress-analytics --verbose >> /app/build/.docker.txt 2>&1 \
				&& flutter pub run build_runner build --delete-conflicting-outputs --release --verbose >> /app/build/.docker.txt 2>&1 \
				&& flutter test --concurrency=6 --dart-define=environment=testing --coverage test/ >> /app/build/.docker.txt 2>&1 \
				&& lcov --summary coverage/lcov.info \
				&& genhtml -o coverage coverage/lcov.info \
				&& date >> /app/build/.docker.txt 2>&1 \
